...
export default function Chat({ otroUsuarioId, nombreOtro, rolOtro = 'Usuario', avatarOtro = null }) {
  ...
  const [tab,     setTab]     = useState('mensajes') // 'mensajes' | 'consultas'
  ...
  const Avatar = ({ url, nombre, size = 40, fontSize = 15 }) => (
    <div
      ...
      style={{
        ...
        flexShrink: 0, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize, fontWeight: 700, color: '#fff', fontFamily: 'DM Sans, sans-serif',
      }}
    >
      {url ? (
    ...
    <div className="chat-layout">
      ...
      <div className="chat-container">
        ...
        <div className="chat-tabs">
          <button
            className={`chat-tab ${tab === 'mensajes' ? 'active' : ''}`}
            onClick={() => setTab('mensajes')}
          >
            💬 Mensajes directos
          </button>
          <button
            className={`chat-tab ${tab === 'consultas' ? 'active' : ''}`}
      ...
      {/* ── SIDEBAR (solo visible en mensajes directos) ── */}
      {tab === 'mensajes' && (
