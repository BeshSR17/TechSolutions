      ...
      <div className="ads-modal ads-modal--lg" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
              ...
              {dias !== null && (
                <span className="ads-badge" style={{
                  ...
                  background: dias < 0 ? 'rgba(239,68,68,0.12)' : dias <= 7 ? 'rgba(245,158,11,0.12)' : 'rgba(16,185,129,0.12)'
                }}>
        ...
        <div className="ads-quickinfo ads-quickinfo--4">
          <div className="ads-qi-item">
            <span className="ads-qi-label">Inicio</span>
            <span className="ads-qi-val">{formatFecha(proyecto.fecha_inicio)}</span>
          </div>
          <div className="ads-qi-item">
            <span className="ads-qi-label">Vencimiento</span>
            <span className="ads-qi-val">{formatFecha(proyecto.fecha_fin)}</span>
          </div>
          <div className="ads-qi-item">
            <span className="ads-qi-label">Tareas</span>
            <span className="ads-qi-val" style={{ color: '#3b82f6', fontWeight: 700 }}>{statsT.total}</span>
          </div>
          <div className="ads-qi-item">
            <span className="ads-qi-label">Avance global</span>
            <span className="ads-qi-val" style={{ color: '#10b981', fontWeight: 700 }}>{avanceTotal}%</span>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {tareas.map(t => {
                const tCfg = {
                  ...
                  'En Progreso': { color: '#3b82f6', bg: 'rgba(59,130,246,0.12)'  },
                  'En Revisión': { color: '#f59e0b', bg: 'rgba(245,158,11,0.12)'  },
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
                      <div style={{ width: '60px' }}>
                        <div className="ads-progress-track">
                          <div className="ads-progress-fill" style={{ width: `${t.avance || 0}%` }} />
                        </div>
...
// ── Modal Form ────────────────────────────────────────────────────────────────
const ModalForm = ({ editando, datos, onChange, onGuardar, onCerrar, enviando, clientes }) => (
  <div className="ads-modal-overlay" onClick={onCerrar}>
      ...
      <form className="ads-form" onSubmit={onGuardar}>
        <div className="ads-form-row ads-form-row--2">
          <div className="ads-form-group">
            ...
            <select className="ads-select" required value={datos.cliente_id} onChange={e => onChange({ ...datos, cliente_id: e.target.value })}>
          ...
          <div className="ads-form-group">
            ...
            <input className="ads-input" placeholder="Nombre del proyecto" required
              value={datos.nombre_proyecto} onChange={e => onChange({ ...datos, nombre_proyecto: e.target.value })} />
          </div>
        </div>
        <div className="ads-form-group">
...
const ProyectosView = () => {
  ...
  const proyectosFiltrados = proyectos.filter(p => {
    ...
    const coincide = p.nombre_proyecto?.toLowerCase().includes(term) ||
  ...
  const handleEliminar = async (id) => {
    if (!window.confirm('¿Eliminar este proyecto y sus datos?')) return
    const res = await apiClient(`/proyectos/${id}`, { method: 'DELETE' })
    if (res.ok) { setProyectoDetalle(null); fetchData() }
  }
  ...
  return (
    <div className="ads-root">
      ...
      <div className="ads-toolbar">
        <input className="ads-search" placeholder="🔍  Buscar proyecto, cliente o descripción..."
      ...
      {loading ? (
        ...
        <div className="ads-grid">
          {proyectosFiltrados.map((p, i) => {
            ...
            const avance = tareasProy.length
              ? Math.round(tareasProy.reduce((a, t) => a + (t.avance || 0), 0) / tareasProy.length)
              : 0
            ...
            return (
              <div key={p.id} className="ads-card" style={{ animationDelay: `${i * 0.04}s` }} onClick={() => abrirDetalle(p)}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <span className="ads-badge" style={{ color: cfg.color, background: cfg.bg }}>
                    <span className="ads-dot" style={{ background: cfg.dot }} />{p.estado}
                  </span>
                ...
                <div>
                  ...
                  {p.descripcion && (
                    <p style={{ color: 'var(--ads-sub)', fontSize: '12.5px', margin: '6px 0 0', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{p.descripcion}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="ads-progress-track">
                    <div className="ads-progress-fill" style={{ width: `${avance}%`, background: cfg.dot }} />
                  </div>
                </div>

                {/* Footer */}
                <div style={{ borderTop: '1px solid var(--ads-border)', paddingTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: 'var(--ads-sub)', fontSize: '11px', fontFamily: 'var(--ads-mono)' }}>
                    {formatFecha(p.fecha_inicio)} → {formatFecha(p.fecha_fin)}
                  </span>
                  {dias !== null && (
                    <span style={{
                      fontSize: '11px', fontWeight: 600, fontFamily: 'var(--ads-mono)',
                      color: dias < 0 ? '#ef4444' : dias <= 7 ? '#f59e0b' : '#10b981'
                    }}>
      ...
      {proyectoDetalle && (
        <ModalProyecto proyecto={proyectoDetalle} onCerrar={() => setProyectoDetalle(null)} onEditar={abrirForm} onEliminar={handleEliminar} />
      )}

      {mostrarForm && (
        <ModalForm editando={!!editandoId} datos={formData} onChange={setFormData} onGuardar={handleGuardar} onCerrar={cerrarForm} enviando={enviando} clientes={clientes} />
      )}
    </div>
  )
}
